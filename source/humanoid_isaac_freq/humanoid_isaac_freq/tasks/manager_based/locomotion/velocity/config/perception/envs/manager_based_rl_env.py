"""Perception-local manager environment state for swap-time termination."""
...
class ManagerBasedRLYMEnv(ManagerBasedRLEnv):
    """Manager environment with the bilateral hip-ordering state."""

    def __init__(self, cfg: ManagerBasedRLEnvCfg, render_mode: str | None = None, **kwargs):
        self.has_not_sweep_time = torch.zeros(cfg.scene.num_envs, device=cfg.sim.device)
        self.prev_left_greater = torch.zeros(
