...
"""Shared 21-DOF frequency-mimic environment configuration components."""
...
ACTUATED_JOINT_NAMES += ["waist_yaw_joint"] + [
...
YAW_JOINT_NAMES = ["left_hip_yaw_joint", "right_hip_yaw_joint", "waist_yaw_joint"]
ROBOT_BASE_LINK = "base_link"
ROBOT_FOOT_LINKS = ["left_ankle_roll_link", "right_ankle_roll_link"]
MAX_VEL = 1.0


# Match DreamLab: --resume selects the fine-tuning stage at module import.
# This flag alone does not alter MDP terms; stage-specific choices are explicit.
_resume_env = os.environ.get("RSL_RL_RESUME")
if _resume_env is None:
    firstStage = 1
else:
    try:
        resume_flag = bool(int(_resume_env))
    except Exception:
        resume_flag = str(_resume_env).lower() in ("1", "true", "yes")
    firstStage = 0 if resume_flag else 1
...
def mimic_joint_action_rate_l2(env, asset_cfg: SceneEntityCfg):
    """Select policy indices by name, not articulation's potentially different order."""
    indices = [ACTUATED_JOINT_NAMES.index(name) for name in asset_cfg.joint_names]
    delta = env.action_manager.action[:, indices] - env.action_manager.prev_action[:, indices]
    return delta.square().sum(dim=-1)


@configclass
class YMBOY21DOFTimeOnlyRewardsCfg:
    ...
    if not firstStage:
        ...
        #     func=mimic_joint_action_rate_l2,
        #     weight=-0.05,
        #     params={
        #         "asset_cfg": SceneEntityCfg("robot", joint_names=ANKLE_JOINT_NAMES, preserve_order=True)
        #     },
...
FREQUENCY_JOINT_SCALES = {
    name: {
        "hip_pitch": 0.55, "hip_roll": 0.10, "hip_yaw": 0.05,
        "knee": 0.85, "ankle_pitch": 0.55, "ankle_roll": 0.25,
    }.get(name.split("_", 1)[1][:-6], 0.25 if name == "waist_yaw_joint" else 0.5)
    for name in ACTUATED_JOINT_NAMES
}

# These physical cross-limb pairings are shared by v0 and v1.
CROSS_LIMB_PHASE_PAIRS = [
    ('left_shoulder_pitch_joint', 'right_hip_pitch_joint'),
    ('right_shoulder_pitch_joint', 'left_hip_pitch_joint'),
]

ALL_FREQUENCY_JOINTS = list(ACTUATED_JOINT_NAMES)
...
OTHER_FREQUENCY_JOINTS = [
    name for name in ALL_FREQUENCY_JOINTS if name not in CORE_FREQUENCY_JOINTS
]


@configclass
class YMBOY21DOFFrequencyMimicEnvCfgBase(LocomotionVelocityEnvCfg):
    """Environment behavior shared by the versioned frequency MDPs."""
    ...
    def __post_init__(self):
        ...
        self.scene.robot = YMBOT_BOY_21DOF_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
